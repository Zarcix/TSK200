...
Test(TSK_LOADER_TESTS, LABEL, .init=loader_label_setup, .fini=loader_label_teardown) {
    ...
    cr_assert_eq(l1Loc, 6);
    cr_assert_eq(l2Loc, 3);
    cr_assert_eq(l3Loc, 0);

    for (int i = 0; i < loader_label_toTest->instructionCount; i++) {
        // Ensure opcodes match
        cr_assert_eq(loader_label_toTest->instructionList[i].operation, expected_opcodes[i]);

        // Ensure src values match. No dest values are expected since all jump instructions have only one arg
        cr_assert_eq(loader_label_toTest->instructionList[i].src.type, expected_src[i].type);

        switch (expected_src[i].type) {
            case PORT: {
                cr_assert_eq(loader_label_toTest->instructionList[i].src.value.dataPort, expected_src[i].value.dataPort);
                break;
            }
            case NUMBER: {
                cr_assert_eq(loader_label_toTest->instructionList[i].src.value.dataVal, expected_src[i].value.dataVal);
                break;
            }
            case STRING: {
                cr_assert_str_eq(loader_label_toTest->instructionList[i].src.value.dataStr, expected_src[i].value.dataStr);
                break;
            }
        }
    }
